...
async def write_variable_values(client, node_ids, values):
    """
    Helper function to write new values to xml nodeset.
    """
...
async def main():
    ...
    async with Client(url=f"opc.tcp://{localhost}:{port}/freeopcua/server/") as client:
        ...
        await subscribe_variables(client, setup_in)
        ...
        await subscribe_variables(client, setup_out)
        ...
        #print(in_nodes[0].nodeid.to_string()) # converts node object to string
        await subscribe_variables(client, subtask_in)
        ...
        await subscribe_variables(client, subtask_out)
